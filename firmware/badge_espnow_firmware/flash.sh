#!/usr/bin/env bash
# Builds and flashes a badge (ESP32-C3)
set -euo pipefail
...
if [ "$CHIP" != "ESP32-C3" ]; then
	echo "device on $PORT identifies as '$CHIP', not ESP32-C3 - refusing to flash badge firmware to it" >&2
	exit 1
fi

echo "flashing badge on $PORT ($CHIP)"
idf.py build
idf.py -p "$PORT" flash
